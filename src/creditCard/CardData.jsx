...
//    {id: 1 , name: <img className="panda" src={panda} alt="" /> , },
//    {id: 2, name: <img className="koala" src={koala} alt="" /> , }
...
function checkMounth(){
    if(Number(cardForm.card_m)>12 || Number(cardForm.card_m)<1 ){
        setRightMounth(false)
    }if(rightMounth == false && (Number(cardForm.card_m)<13 && Number(cardForm.card_m)>1 )){
        setRightMounth(true)
...
function userHandler(name){
    // let userName = name.props.className
    ...
    return (
        ...
        <div className='flex'>
         {rightMounth == false && <h1 className='text-rose-700 text-lg'>Incorrect month</h1>}   
        <h1 className={`text-lg text-zinc-300 ${rightMounth == false ? "pl-16" : 'pl-48' }`}>Card details</h1>
        </div>
        ...
        {/* <h1 className='text-lg text-zinc-700 pt-1'>/</h1>  */}
         <input type="text"  onKeyPress={(event) => {
                          ...
                          className=" w-7 rounded-md border-0 pl-1.5 pr-2  text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:outline-none focus:ring-inset focus:ring-rose-200 sm:text-sm sm:leading-6" 
                          name={'card_y'}
                          id={2}
